...
- [ ] T0.10 Monorepo structure — `backend/`, `frontend/`, tooling, CI entry point
- [ ] T0.11 [P] JWT auth issuing verified claims for athlete and coach roles
- [ ] T0.12 [P] Import-boundary test asserting `risk_engine` imports nothing from `orchestrator`,
      `api`, `audit`, `consent`, `explanation`, or any network/LLM library —
      `backend/tests/unit/test_import_boundary.py` (Principle I; guards T1.*)

### Schema and roles

- [ ] T0.1 PostgreSQL schema — Athlete, ConsentRecord, HealthMetric, RuleSetVersion,
      RiskAssessment, PlanProposal, ApprovalRecord, AuditEntry, Escalation —
      `backend/migrations/001_schema.sql`
- [ ] T0.2 `ENABLE` + `FORCE ROW LEVEL SECURITY` and purpose-scoped policies on all health tables
      (ADR-0001 condition 1) — `backend/migrations/002_rls.sql`
- [ ] T0.3 Non-owner application role: no table ownership, no `BYPASSRLS`, not superuser
      (ADR-0001 condition 2) — `backend/migrations/003_roles.sql`
- [ ] T0.4 Transaction-scoped identity propagation — `SET LOCAL` from the **verified JWT claim**,
      never a client-supplied header; policies read it — `backend/src/api/session_identity.py`

### Audit substrate

- [ ] T0.6 Append-only audit triggers + hash chaining —
      `backend/migrations/004_audit_triggers.sql`
- [ ] T0.7 Serialise concurrent audit appends (advisory lock on the chain) — ADR-0001 condition 4
- [ ] T0.8 **Decide and document** audit write durability under caller rollback, then implement —
...
- [ ] T0.5 [P] Read health data with session identity **unset** → zero rows (SC-006)
- [ ] T0.13 [P] Read another athlete's health data as an authenticated athlete → zero rows (SC-006)
- [ ] T0.14 [P] Read health data **bypassing application code**, connecting directly as the
      application role with no covering consent → zero rows (SC-006, FR-011)
- [ ] T0.15 [P] `UPDATE` and `DELETE` on `AuditEntry` as the application role → fails, and the
      attempt is itself recorded (FR-009)
- [ ] T0.16 [P] Concurrent audit appends under load produce an unforked, verifiable chain (T0.7)
- [ ] T0.17 [P] Audit entry survives / behaves as decided when the caller transaction rolls back
      (T0.8)

### Seed

- [ ] T0.9 Seed realistic women athlete data, consent records, and coach assignments — including
...
- [ ] T1.1 Versioned Rule Set with content-hash identity — `backend/src/risk_engine/version.py`
- [ ] T1.2 [US1] Sex-specific factors: cycle phase, contraception, iron status, RED-S indicators,
      ACL mechanics — `backend/src/risk_engine/factors.py` (FR-015)
- [ ] T1.9 [US1] **Three distinct cycle states** — not recorded / recorded as absent / suppressed
      by contraception — modelled as separate values, never collapsed (FR-016). Absent menses is a
      RED-S signal, not missing data.
- [ ] T1.3 [US1] Per-metric freshness windows — `backend/src/risk_engine/freshness.py` (FR-022)
- [ ] T1.4 [US1] Confidence computation and 0.7 floor — `backend/src/risk_engine/confidence.py`
      (FR-023)
- [ ] T1.5 [US1] Risk bands — provisional 0–39 low, 40–59 moderate, 60–79 elevated, 80–100 high.
      Mark provisional in code, not only in the spec.
- [ ] T1.16 [US1] Cricket bowling workload as a first-class load input — overs bowled, spell
      density, consecutive playing days — with the rule structure kept sport-agnostic (FR-026)
- [ ] T1.10 [US1] Evidence returned as part of the engine's result value — contributing metrics
...
- [ ] T1.11 [US1] Refusal result type carrying a machine-readable reason, the offending input, and
      the computed confidence (FR-006, FR-007, FR-023)

### Phase 1 tests — written first, failing

- [ ] T1.6 Unit tests for every rule
- [ ] T1.7 Mutation testing over safety rules — a removed or inverted rule MUST fail a test
- [ ] T1.8 [P] Refusal path tests: missing, stale, and low-confidence, for each condition
      introduced here (SC-004)
- [ ] T1.12 [P] Boundary tests on **each side** of all six freshness windows — refusal one
      interval past, score one interval inside (SC-012)
- [ ] T1.13 [P] Repeatability trials — identical inputs + rule version → identical score (SC-008,
      Principle I)
- [ ] T1.14 [P] No male-default substitution: with a sex-specific input absent, assert the output
      states the absence and no default value was applied (FR-017, SC-002)
- [ ] T1.15 [P] Sex-specific citation: with data present, every score cites ≥1 sex-specific factor
      (SC-001, EQ-001)
...
- [ ] T2.1 [US1] Manual metric ingestion — capture time distinct from ingestion time; source
      recorded; staleness measured against **capture** time (FR-024, FR-025)
- [ ] T2.2 [US1] Risk assessment endpoint calling the pure engine
- [ ] T2.6 [US1] Audit entry written before every response, including refusals (FR-008)
- [ ] T2.7 [US1] Escalation on refusal — to the assigned coach, or to the athlete when unassigned
      (SA-001)
- [ ] T2.8 [US1] Superseded score retained in the audit record when late-arriving data forces
      recomputation (FR-019)
- [ ] T2.4 [US2] Plan proposal generation and state machine — proposed / approved / rejected /
      superseded
- [ ] T2.5 [US2] Tiered approval gate — coach mandatory at elevated and high; athlete may
      self-approve below (FR-021, FR-004)
- [ ] T2.9 [US2] Approval invalidation — an approval does not survive modification of the approved
      artifact; re-approval required (FR-005)
- [ ] T2.3 [US2] Boundary re-check — narrative contradicting the deterministic verdict returns an
      error, not the narrative (FR-003) — `backend/src/orchestrator/boundary.py`
- [ ] T2.13 [US2] Diagnosis / treatment guard — a request whose output would constitute diagnosis
      or treatment is refused and escalated (FR-020, SA-005)
- [ ] T2.14 [US4] Consent grant and withdrawal — purpose-scoped, revocable; withdrawal stops
      future processing within the declared window while retaining prior audit entries (FR-012)
- [ ] T2.15 Log, trace, and error scrubbing — no raw health data in any of them (FR-018)

### Phase 2 tests — written first, failing

- [ ] T2.10 [P] [US2] Activation attempted through every path with no approval record → refused
      and the refused attempt recorded (SC-003)
- [ ] T2.11 [P] [US2] Athlete self-approval of an elevated/high-risk plan → refused (SC-011)
- [ ] T2.12 [P] [US2] Athlete with **no assigned coach** + elevated risk → plan stays inactive,
      reason names the absent coach approver. Fails closed; does **not** fall back to
      self-approval (US2 scenario 8)
- [ ] T2.16 [P] [US4] Consent withdrawal stops processing within the declared window (SC-010)
- [ ] T2.17 [P] [US1] Every score response carries contributing metrics with timestamps and the
      rule versions that fired (SC-007)
- [ ] T2.18 [P] Audit entry exists **before** the response is observable by the caller (FR-008)

---

## Phase 3 — Independence and hardening

- [ ] T3.5 [US1] **Pin the exact LLM model ID** — do this *before* T3.6, not after. FR-008
...
- [ ] T3.6 [US1] Explanation service — narrative only, generated from the same computation that
      produced the score (FR-002, FR-014)
- [ ] T3.7 [US1] Explanation-unavailable fallback — the score and its structured evidence are
      still delivered when narrative generation fails (spec edge case)
- [ ] T3.8 [P] Test: LLM output cannot alter any score, threshold, or band (FR-002, Principle I)
- [ ] T3.2 [US3] Independent integrity check — verifiable with read access and the chain rule
      alone, without trusting the application (FR-010)
- [ ] T3.9 [P] [US3] Altered-entry detection trials → detected 100% (SC-005)
- [ ] T3.3 End-to-end refusal tests across the full stack — integration only; the per-path unit
...
- [ ] T4.1 [US1] Athlete dashboard — score, evidence, and **refusal states as first-class UI**
      with the stated reason; not an error toast
- [ ] T4.7 [P] [US1] Evidence display component — metrics with capture times, rules with versions,
      sex-specific factors surfaced
- [ ] T4.2 [P] [US2] Coach approval view — risk band shown, and why coach approval is required
- [ ] T4.3 [US2] Plan proposal review, approve, reject
- [ ] T4.8 [P] [US4] Consent management view — grant and withdraw by purpose
- [ ] T4.10 [P] Accessibility — WCAG 2.2 AA across athlete and coach surfaces; score, evidence and
      refusal reason each completable by keyboard and screen reader alone (FR-027, SC-013)
- [ ] T4.11 [P] Second language covering refusal reasons and evidence labels, not only navigation
      (FR-028, SC-014)
- [ ] T4.4 Seed demo data covering happy path **and** refusal path scenarios

### Submission deliverables (competition rules, not engineering)

- [ ] T4.12 **USP statement** — one page: what ShePeak does that existing tools do not, and why
...
- [ ] T4.13 Written summary, max 2 pages — problem statement, proposed solution, impact for women
...
- [ ] T4.14 **Record and edit the demo video**, 3 min hard maximum — the script is not the
      deliverable
- [ ] T4.15 Submission accuracy review — verify no artifact claims independent auditability
